...
export const CardContainer: React.FC<CardContainerProps> = ({
	...
	const widthClass = twMerge(["w-full", CardWidth[widthLimit]])
...
export const BlockLinkCard = ({
	...
	CustomLink?: React.ComponentType<any>
	customlinkprop?: any
	widthLimit?:CardContainerProps["widthLimit"]
}) => {
	...
	return (
		...
		className="text-gray-800 transition-colors hover:bg-gray-200/50 dark:text-white dark:hover:bg-zinc-700/50">
			{CustomLink ? (
				<CustomLink {...customlinkprop} className={linkClasses}>
					{linkContent}
				</CustomLink>
			) : (
				<a
					...
					href={url || "#"}
					target="_blank"
					{...(customlinkprop as React.AnchorHTMLAttributes<HTMLAnchorElement>)}
				>
					{linkContent}
				</a>
			)}
